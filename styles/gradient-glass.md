...
你是一位专家级UI UX演示设计师，请生成 16:9 横版宽屏的演示文稿幻灯片。请根据视觉平衡美学，自动在封面、网格布局或数据可视化中选择一种最完美的构图。
...
渲染质量要求：虚幻引擎5渲染，超细节纹理，UI设计感，UX界面，Dribbble热门趋势，设计奖获奖作品。
...
- 比例：16:9 → size=`1536x1024`；竖版 9:16 → `1024x1536`；方图 → `1024x1024`
- 质量：`quality=high`（也支持 low / medium / auto）
- 返回：`response_format=b64_json`，base64 解码后落盘
