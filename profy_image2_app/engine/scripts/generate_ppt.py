...
def resolve_resource_path(path_value: str, *, default_subdir: str = "") -> str:
    """Resolve a user-facing resource path across source and packaged layouts.

    Source checkout layout keeps styles/templates at repo root. Profy packages a
    skill's auxiliary text files under `{baseDir}/references`, next to `scripts/`.
    This resolver supports both without making users remember the runtime layout.
    """
    ...
    if default_subdir:
        candidates.extend([
            SCRIPT_DIR.parent / default_subdir / path,
            SCRIPT_DIR.parent / default_subdir / path.name,
        ])

    candidates.append(SCRIPT_DIR.parent / "references" / path.name)
...
def find_and_load_env() -> bool:
    ...
    2. <script_dir>/.env
    3. <script_dir>/../.env  -- the skill root in packaged layouts
    4. ~/.codex/skills/gpt-image2-ppt-skills/.env  -- Codex install
    5. ~/.claude/skills/gpt-image2-ppt-skills/.env  -- default Claude Code install
    6. ~/skills/gpt-image2-ppt/.env  -- default OpenClaw install
...
def load_style_template(style_path: str) -> str:
    ...
    style_path = resolve_resource_path(style_path, default_subdir="styles")
...
def generate_viewer_html(
    ...
    template_path = resolve_resource_path(template_path, default_subdir="templates")
...
def main() -> None:
    ...
    style_template = ""
    if args.style:
        style_path = resolve_resource_path(args.style, default_subdir="styles")
